package  com.core.manager
{
	...
	public class FileManagers extends EventDispatcher
	{
		...
		private function onUnitLoadComplete(data:UnitLoader):void
		{
			...
			var slashArray:Array = data.url.split("/");
			var unitID:String = (slashArray[slashArray.length-1] as String).split(".")[0];
//			var unitView:UnitView = UnitCreator.instance.createUnitView(unitID);
//			
//			unitVector.push(unitView);
			BattleField.instance.createDefaultTeam();
		}	
		
		private var unitVector:Vector.<UnitView> = new Vector.<UnitView>();
